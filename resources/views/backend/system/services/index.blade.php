                    ...
                    <form method="get" action="{{ route('services.index') }}">
                        <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                                                ...
                                                <select name="service_category_id" class="form-select">
                                                    ...
                                                    @foreach($categories as $category)
                                                        <option value="{{ $category->id }}"
                                                                @if (request('service_category_id') == $category->id) selected @endif>
                            ...
                            <table class="datatables table dataTable no-footer dtr-column" id="DataTables_Table_0"
                                ...
                                @endif
                                @foreach ($thisDatas as $serviceKey => $service)
                                    <tr>
                                        <td>{{ $serviceKey + 1 }}</td>
                                        <td>{{ $service->title }}</td>
                                        <td>{{ $service->position }}</td>
                                        <td><span class="badge @if ($service->status) bg-label-primary @else bg-label-secondary @endif me-1">
                                                @if ($service->status)
                                                    {{ __('Active') }}
                                                @else
                                                    {{ __('Inactive') }}
