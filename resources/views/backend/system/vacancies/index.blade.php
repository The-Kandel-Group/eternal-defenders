                ...
                <div class="card-datatable table-responsive">
                    <form method="get" action="{{ route('vacancies.index') }}">
                        <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                                        ...
                                        <div class="dt-buttons btn-group flex-wrap">
                                            <div class="btn-group">
                                                ...
                                                @if(hasPermission('/'.str_replace(' ','-',strtolower($title)),'post'))
                                                    <a class="btn add-new btn-primary text-white" href="{{ route('vacancies.create') }}">
                            ...
                            <table class="datatables-vacancies table dataTable no-footer dtr-column" id="DataTables_Table_0">
                                ...
                                @foreach ($thisDatas as $index => $post)
                                    <tr>
                                        ...
                                        <td>{{ $post->slug }}</td>
                                        <td>
                                            <span class="badge @if ($post->status) bg-label-primary @else bg-label-secondary @endif me-1">
                                                @if ($post->status)
                                                    {{ __('Active') }}
                                                @else
                                                    {{ __('Inactive') }}
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        @if(hasPermission('/'.str_replace(' ','-',strtolower($title)).'/*','put'))
                                                            <a class="dropdown-item" href="{{ route('vacancies.edit', $post->id) }}">
                                                                <i class="ti ti-pencil me-1"></i>{{ __('Edit') }}
                                                            </a>
                                                        @endif
                                                        @if(hasPermission('/'.str_replace(' ','-',strtolower($title)).'/*','delete'))
                                                            <a href="#" class="dropdown-item delete-button" data-bs-toggle="modal" data-actionurl="{{ route('vacancies.destroy', $post->id) }}" data-bs-target="#deleteModal">
