        ...
        <div class="card mb-4">
            ...
            <div class="card">
                ...
                <div class="card-datatable table-responsive">
                    <form method="get" action="{{ route('core-values.index') }}">
                        <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                                        ...
                                        <div class="dt-buttons btn-group flex-wrap">
                                            <div class="btn-group">
                                                ...
                                                @if(hasPermission('/'.str_replace(' ','-',strtolower($title)),'post'))
                                                    <a class="btn add-new btn-primary text-white" href="{{ route('core-values.create') }}">
                            ...
                            <table class="datatables table dataTable no-footer dtr-column" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
                                ...
                                @endif
                                @foreach ($thisDatas as $postKey => $post)
                                    <tr>
                                        <td>{{ $postKey + 1 }}</td>
                                        <td>{{ $post->title }}</td>
                                        <td><span class="badge @if ($post->status) bg-label-primary @else bg-label-secondary @endif me-1">
                                                @if ($post->status)
                                                    {{ __('Active') }}
                                                @else
                                                    {{ __('Inactive') }}
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        @if(hasPermission('/' . str_replace(' ','-',strtolower($title)) . '/*', 'put'))
                                                            <a class="dropdown-item" href="{{ route('core-values.edit', $post->id) }}"><i class="ti ti-pencil me-1"></i>{{ __('Edit') }}</a>
                                                        @endif
                                                        @if(hasPermission('/' . str_replace(' ','-',strtolower($title)) . '/*', 'delete'))
                                                            <a href="#" class="dropdown-item delete-button" data-bs-toggle="modal" data-actionurl="{{ route('core-values.destroy', $post->id) }}" data-bs-target="#deleteModal">
